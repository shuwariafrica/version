...
import africa.shuwari.version.codecs.jsoniter.given
...
class JsoniterCodecsSuite extends munit.FunSuite:
  ...
  val validPreReleasePairs: List[(String, PreRelease)] = List(
    ...
    """{"classifier":"m","number":1}""" -> PreRelease.milestone(PreReleaseNumber.wrap(1)),
    """{"classifier":"alpha","number":1}""" -> PreRelease.alpha(PreReleaseNumber.wrap(1)),
    """{"classifier":"beta","number":1}""" -> PreRelease.beta(PreReleaseNumber.wrap(1)),
    """{"classifier":"rc","number":1}""" -> PreRelease.releaseCandidate(PreReleaseNumber.wrap(1))
  )

  val validVersionPairs: List[(String, Version)] =
    def appendPreRelease(kv: (String, PreRelease)) =
      s"""{"majorVersion":1,"minorVersion":10,"patchNumber":1,"preRelease":${kv._1}}""" -> Version(
        MajorVersion.wrap(1),
        MinorVersion.wrap(10),
        PatchNumber.wrap(1),
        Some(kv._2))
    val finalVersion =
      s"""{"majorVersion":1,"minorVersion":10,"patchNumber":1}""" -> Version(
        MajorVersion.wrap(1),
        MinorVersion.wrap(10),
        PatchNumber.wrap(1))
  ...
  test("Jsoniter decoding of MajorVersion instances") {
    val validMajorVersion1 = "1" -> MajorVersion.wrap(1)
  ...
  test("Jsoniter decoding of Minor instances") {
    val validMajorVersion1 = "1" -> MinorVersion.wrap(1)
    val invalidVersionNumber = "-1"
  ...
  test("Jsoniter decoding of PatchNumber instances") {
    val validMajorVersion1 = "1" -> PatchNumber.wrap(1)
    val invalidVersionNumber = "-1"
  ...
  test("Jsoniter decoding of PreReleaseNumber instances") {
    val validMajorVersion1 = "1" -> PreReleaseNumber.wrap(1)
    val invalidVersionNumber = "-1"
  ...
  test("Jsoniter decoding of Version instances") {
    ...
    val invalidInput =
      s"""{"majorVersion":1,"minorVersion":10,"patchNumber":1,"preRelease":{"classifier":"snapshot","number":1}}"""
