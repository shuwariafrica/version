...
[[version.semver.SemVer.Formatter]] provides configurable variants:

| Formatter                  | Behaviour                              | Example Output                            |
|----------------------------|----------------------------------------|-------------------------------------------|
| `Formatter.standard`       | Same as `v.show`                       | `1.2.3-SNAPSHOT`                          |
| `Formatter.extended`       | Includes metadata, SHA truncated to 7  | `1.2.3-SNAPSHOT+sha1234567`               |
| `Formatter.full`           | Complete metadata without truncation   | `1.2.3-SNAPSHOT+sha1234567890abcdef`      |

```scala
import version.semver.*

val v = SemVer.parseUnsafe("1.2.3-SNAPSHOT+sha1234567890abcdef")
v.show                              // "1.2.3-SNAPSHOT"
SemVer.Formatter.extended.format(v) // "1.2.3-SNAPSHOT+sha1234567"
SemVer.Formatter.full.format(v)     // "1.2.3-SNAPSHOT+sha1234567890abcdef"
```

#### Custom Formatters

Implement [[version.semver.SemVer.Formatter]] for full control:
