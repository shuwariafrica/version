...
class MetadataSuite extends FunSuite:
  ...
  test("identifiers within the permitted charset are accepted and rendered dot-separated") {
    assertEquals(Metadata.from(List("build", "0123", "a-b")).map(_.show), Right("build.0123.a-b"))
  }

  test("a numeric identifier may carry a leading zero") {
    assertEquals(Metadata.from(List("01")).map(_.identifiers), Right(List("01")))
  }

  test("an empty identifier list is rejected") {
    assertEquals(Metadata.from(Nil), Left(InvalidMetadata(Nil)))
  }

  test("an empty identifier is rejected") {
    assertEquals(Metadata.from(List("build", "")), Left(InvalidMetadata(List("build", ""))))
  }

  test("a character outside the ASCII alphanumerics and hyphen is rejected") {
    assertEquals(Metadata.from(List("build_1")), Left(InvalidMetadata(List("build_1"))))
    assertEquals(Metadata.from(List("build.1")), Left(InvalidMetadata(List("build.1"))))
  }
  ...
  test("a letter outside ASCII is rejected, matching what the parser accepts") {
    ...
    assertEquals(Metadata.from(List(accented)), Left(InvalidMetadata(List(accented))))
    assert(SemVer.parse(s"1.0.0+$accented").isLeft)
  }

  test("unsafe construction throws what safe construction reports") {
    intercept[InvalidMetadata](Metadata.fromUnsafe(List("build_1")))
  }
